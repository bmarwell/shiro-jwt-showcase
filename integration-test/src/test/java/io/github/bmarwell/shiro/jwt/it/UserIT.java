...
public class UserIT extends AbstractShiroJaxRsIT {
  ...
  public static void getAdminToken() {
    UserIT.adminToken = getToken(List.of("user"));
  }

  @Test
  @Order(1)
  public void testGetTroopersAsMod() {
  ...
  public void testGetTroopersAsAdminAtEnd() {
    ...
    assertThat(stormtroopers)
        .hasSize(INITIAL_COUNT)
        .allMatch(st -> st.id().startsWith("FN-"), "id starts with 'FN-'")
        .noneMatch(st -> st.planetOfOrigin().equals("Vulcan"))
        .noneMatch(st -> st.planetOfOrigin().equals("Qo’noS"))
    ;
  }
